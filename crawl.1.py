...
def fetchItem(itemurl, record):
	...
	# Task.3. Release time
	try:
		release = soup.find(id='ProductInfoReleaseDate')
		#release = soup.find(id='releaseDate_feature_div')
		print release
		#	{'class' : lambda x: x and re.search('(\s|^)header-price(\s|$)', x)})
		record['ReleaseDate'] = release.string.strip()
	except:
		record['ReleaseDate'] = ''
	# Task 1. Sales Rank
...
#			newOffer = soup.find(id='newOfferAccordionRow')
	...
	record['Name'] = ''
	try:
		block = soup.find(id='ppd-center') # music
		feature = soup.find(id='title_feature_div')
		print feature
		name = soup.find('h1', class_='a-size-large a-spacing-micro')
		print name
...
def amazoncrawl(startUrl, keywords, path):
	...
	for key in keywords:
		...
		urlfp = open(path.strip() + '/' + urlfilename, 'w')
...
def testItem():
	...
	itemurl='http://www.amazon.com/Talking-Body-Explicit/dp/B00NOII3NG'
